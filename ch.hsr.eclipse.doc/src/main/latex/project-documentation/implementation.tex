...
\subsubsection{Run Inside a Separate Job}
\textit{Toggle Function Definition} does not use modal dialogs and therefore 
could invoked multiple times in parallel. However, this 

Multiple instances of The \texttt{ToggleRefactoringRunner} must not TODO:***CONTINUE***

\subsubsection{Support Undoing Changes}
TODO:***CONTINUE***
%The \texttt{ToggleRefactoringRunner} has to take care of registering undo 
%changes by itself. Listing \ref{runner} shows the simplified process without 
%error checking. \texttt{createChange} first 

%The changes are collected, executed 
%and registered at the undo manager.
